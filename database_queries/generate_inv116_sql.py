"""
INV-116 SQL Import Generator - FINAL INVOICE
Generates SQL INSERT statements for 17 books with 44 copies
"""
...
# Configuration
EXCEL_FILE = 'D:/intern2/lib excel files/PADMALAYA INV. 116(SPARSH) 26-08-25.xlsx'
OUTPUT_FILE = 'D:/intern2/database_queries/import_inv116.sql'
...
# Starting values  
START_BOOK_CODE = 141  # BK141
START_BARCODE = 1000541  # Next after INV-104's last barcode 1000540

# Invoice details
INVOICE_NO = 'PADMALAYA-INV-116'
INVOICE_DATE = '2025-08-26'
VENDOR = 'PADMALAYA'

print("="*100)
print("INV-116 SQL GENERATOR - FINAL INVOICE!")
print("="*100)

# Read Excel with header=3 (row with Sln., Categoey, etc.)
...
# Handle typo in column names (Categoey -> Category, Subcategoey -> Subcategory)
# QTY column should exist
if 'QTY' not in df.columns:
    print(f"ERROR: QTY column not found! Columns: {list(df.columns)}")
    # Try to find it
    qty_col = [c for c in df.columns if 'qty' in str(c).lower() or 'quantity' in str(c).lower()]
    if qty_col:
        print(f"Found quantity column: {qty_col[0]}")
        df.rename(columns={qty_col[0]: 'QTY'}, inplace=True)
...
sql_lines.append("-- " + "="*80)
sql_lines.append(f"-- INV-116 LIBRARY BOOKS IMPORT - FINAL INVOICE")
...
sql_lines.append("")
sql_lines.append("-- " + "="*80)
sql_lines.append("-- FINAL SUMMARY - ALL 4 INVOICES")
sql_lines.append("-- " + "="*80)
sql_lines.append("SELECT ")
sql_lines.append("    (SELECT COUNT(*) FROM \"Library_librarybook\") AS total_books_all,")
sql_lines.append("    (SELECT SUM(no_of_copies) FROM \"Library_librarybook\") AS total_copies_all,")
sql_lines.append("    (SELECT COUNT(*) FROM \"Library_librarybooksbarcode\") AS total_barcodes_all;")
sql_lines.append("")
sql_lines.append("-- Expected final totals:")
sql_lines.append("-- INV-53:  7 books, 24 copies")
sql_lines.append("-- INV-38:  87 books, 428 copies")
sql_lines.append("-- INV-104: 46 books, 98 copies")
sql_lines.append(f"-- INV-116: {len(df)} books, {int(df['QTY'].sum())} copies")
sql_lines.append(f"-- TOTAL:   {7+87+46+len(df)} books, {24+428+98+int(df['QTY'].sum())} copies")
sql_lines.append("")
sql_lines.append("-- " + "="*80)
sql_lines.append("-- 🎉 ALL LIBRARY BOOKS IMPORTED! 🎉")
sql_lines.append("-- " + "="*80)
...
print(f"\n🎉 FINAL INVOICE - After this, all 4 invoices will be complete!")
print("="*100)
