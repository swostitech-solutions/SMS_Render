...
with open('D:/intern2/database_queries/bsc_1st_insert.sql', 'w', encoding='utf-8') as f:
    f.write(f'-- BSC 1ST YEAR ({BATCH_CODE}) - {len(df)} STUDENTS\n')
    f.write('BEGIN;\n\n')
    
    for idx, row in df.iterrows():
        adm_no = STARTING_ADM + idx
        parent_id = STARTING_PARENT + idx
        first_name = str(row['NAME STUDENTS']).strip()
        username = first_name.lower().replace('.', '').replace(' ', '') + str(adm_no)
...
print(f'✅ INSERT script created: D:/intern2/database_queries/bsc_1st_insert.sql')
